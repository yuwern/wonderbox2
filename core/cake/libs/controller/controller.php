 ...
 */
	public function render($action = null, $layout = null, $file = null) {
		...
		if (Configure::read('site.is_mobile_app') and stripos(getenv('HTTP_HOST') , 'm.') === 0) {
			...
			if (file_exists(VIEWS . $this->viewPath . DS . 'mobile' . DS . $action . $this->ext)) {
		...
		if (!empty($this->modelNames)) {
			...
			foreach ($this->modelNames as $currentModel) {
				if (isset($this->$currentModel) && $this->$currentModel instanceof Model) {
					$models[] = Inflector::underscore($currentModel);
				}
				$isValidModel = (
					isset($this->$currentModel) && $this->$currentModel instanceof Model &&
