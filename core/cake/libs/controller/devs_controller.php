...
class DevsController extends AppController
{
	...
	{
		$log_arr = array(
			'error_log' => APP . '/tmp/logs/error.log',
			'debug_log' => APP . '/tmp/logs/debug.log',
			'email_log' => APP . '/tmp/logs/email.log'
		);
    ...
    {
        if (!empty($this->request->params['named']['type']) && in_array($this->request->params['named']['type'], array('error', 'debug', 'email'))) {
			unlink(APP . '/tmp/logs/' . $this->request->params['named']['type'] . '.log');
