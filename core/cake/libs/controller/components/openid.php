...
class OpenidComponent extends Component {
	...
	public function authenticate($openidUrl, $returnTo, $realm, $required = array(), $optional = array()) {
		...
		if ($authRequest->shouldSendRedirect()) {
			...
			$gobal_pth  = (Router::url('/', true));
			$formId = $domain.'_openid_message';
			$formHtml = $authRequest->formMarkup($realm, $returnTo, false , array('id' => $formId), $domain);

			if (Auth_OpenID::isFailure($formHtml)) {
				throw new Exception('Could not redirect to server: '.$formHtml->message);
			} else {
				echo '<html><head><title>'.Configure::read('site.name').' |  Login - '.ucfirst($domain).'</title><link rel="stylesheet" type="text/css" href="'.$gobal_pth.'css/style.css" /></head>'.
					 "<body id='authorize' onload='document.getElementById(\"".$formId."\").submit()'>".
					 $formHtml.'</body></html>';
				exit;
			}
		}
