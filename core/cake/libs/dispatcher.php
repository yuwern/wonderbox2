 ...
 */
	public function dispatch(CakeRequest $request, $additionalParams = array()) {
		...
		if (($request->requested || $request->is('ajax')) && permanentCached($this->here)) {
			return;
		}
...
// another fix in router.php#851
        $t_action = $this->request->params['action'];
        if (!empty($this->request->params['prefix'])) {
			$t_action = substr($this->request->params['action'], strlen($this->request->params['prefix'] . '_'));
        }
