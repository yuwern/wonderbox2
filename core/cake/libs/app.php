 ...
 */
	private static function __find($file, $recursive = true) {
		...
		$core_file_map = array(
			'controller.php' => LIBS . 'controller',
			'component.php' => LIBS . 'controller' . DS . 'components',
			'component_collection.php' => LIBS . 'controller',
			'view.php' => LIBS . 'view',
			'helper_collection.php' => LIBS . 'view',
			'helper.php' => LIBS . 'view',
			'behavior_collection.php' => LIBS . 'model',
			'model_behavior.php' => LIBS . 'model',
			'connection_manager.php' => LIBS . 'model',
			'dbo_mysql_ex.php' => LIBS . 'model' . DS . 'datasources' . DS . 'dbo',
			'dbo_source.php' => LIBS . 'model' . DS . 'datasources',
		);
		...
		foreach (self::$search as $path) {
			...
			if (in_array($file, array_keys($core_file_map))) {
				if (self::__load($core_file_map[$file] . DS . $file)) {
					return $core_file_map[$file] . DS;
				}
			} else {
