 ...
 */
	...
	private function _permanentCache($is_flash_message) {
		if (Configure::read('PermanentCache.check') === true && !in_array($_SERVER['REQUEST_METHOD'], array('POST', 'PUT', 'DELETE')) && $is_flash_message === false) {
			$permanentCacheAction = $this->permanentCacheAction;
			if (is_array($permanentCacheAction)) {
				$keys = array_keys($permanentCacheAction);
				$index = null;
				if (!empty($this->request->params['prefix']) && $this->request->params['prefix'] == 'admin' && Configure::read('PermanentCache.have_sub_admin') === false) {
				...
				if (!empty($index)) {
					$cache = strtolower(Inflector::slug($this->request->here));
					if (isset($is_admin) || (isset($permanentCacheAction[$index]['is_admin_url']) && ($permanentCacheAction[$index]['is_admin_url'] === true || preg_match('/' . $permanentCacheAction[$index]['is_admin_url'] . '/', $this->request->here)) && !empty($_SESSION['Auth']['User']['user_type_id']) && $_SESSION['Auth']['User']['user_type_id'] == ConstUserTypes::Admin)) {
						$cache .= '_admin';
					} elseif (isset($permanentCacheAction[$index]['is_loggedin_and_usertype_url']) && ($permanentCacheAction[$index]['is_loggedin_and_usertype_url'] === true || preg_match('/' . $permanentCacheAction[$index]['is_loggedin_and_usertype_url'] . '/', $this->request->here)) && !empty($_SESSION['Auth']['User']['user_type_id'])) {
						$cache .= '_usertype_' . $_SESSION['Auth']['User']['user_type_id'];
					} elseif (isset($permanentCacheAction[$index]['is_user_specific_url']) && ($permanentCacheAction[$index]['is_user_specific_url'] === true || preg_match('/' . $permanentCacheAction[$index]['is_user_specific_url'] . '/', $this->request->here)) && !empty($_SESSION['Auth']['User']['id'])) {
						$cache .= '_user_' . $_SESSION['Auth']['User']['id'];
					} elseif (isset($permanentCacheAction[$index]['is_loggedin_url'])) {
						$cache .= '_loggedin';
					} elseif (isset($permanentCacheAction[$index]['is_public_url'])) {
						$cache .= '_public';
					}
					if (!empty($this->request->params['requested']) || !empty($this->request->params['isAjax'])) {
						$cache .= '_requested';
					}
					if (!empty($_COOKIE[Configure::read('PermanentCache.cookie')])) {
						$cache .= '_' . $_COOKIE[Configure::read('PermanentCache.cookie')];
					} else {
						$cache .= '_' . Configure::read('PermanentCache.default_language');
					}
					if (isset($permanentCacheAction[$index]['is_view_count_update']) && $permanentCacheAction[$index]['is_view_count_update'] === true) {
						$cache .= '_updateviews' . $this->viewVars[Inflector::variable(Inflector::singularize($this->request->params['controller']))][Inflector::classify($this->request->params['controller'])]['id'];
					}
					$cache .= '_' . Inflector::classify($this->request->params['controller']);
					$cache .= '.php';
					cache('views' . DS . $cache, $this->output);
				}
			}
		}
	}
