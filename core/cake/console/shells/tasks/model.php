 ...
 */
	protected function _interactive() {
		...
		$displayField = $tempModel->hasField(array('username', 'name', 'title', 'display_name'));
		if (!$displayField) {
			$displayField = $this->findDisplayField($tempModel->schema());
		}
		$isSlug = (in_array('slug', array_keys($fields)));
		$isOrder = (in_array('display_order', array_keys($fields)));
		...
		if (strtolower($looksGood) == 'y') {
			$vars = compact('associations', 'validate', 'primaryKey', 'useTable', 'displayField', 'isSlug', 'isOrder');
		...
		while ($anotherValidator == 'y') {
			...
			if ($choice != $defaultChoice) {
				if (is_numeric($choice) && isset($this->_validations[$choice])) {
					$validate['rule' . $j]['rule'] = $this->_validations[$choice];
				} else {
					$validate['rule' . $j]['rule'] = $choice;
				}
				$choiceAllowEmpty = $this->in(__('Is allow empty?', true), array('y','n'), 'n');
    			$validate['rule' . $j]['allowEmpty'] = $allowEmpty;
                if ((strtolower($choiceAllowEmpty) == 'n' || strtolower($choiceAllowEmpty) == 'no')) {
    			     $validate['rule' . $j]['allowEmpty'] = 'false';
    		    }
    		    $validate['rule' . $j]['message'] = $this->in(__('Enter validation message', true), null, 'Required');
			}
	...
	public function findBelongsTo(&$model, $associations) {
		...
		foreach ($fields as $fieldName => $field) {
			...
			if ($fieldName != $model->primaryKey && $fieldName != 'parent_id' && $offset !== false) {
				...
				$tmpOtherModel = new Model(array(
					'name' => $tmpModelName
				));
				$relatedModelFields = $tmpOtherModel->schema(true);
				$_tmpCounterCache = false;
				if (!empty($relatedModelFields)) {
					foreach($relatedModelFields as $relatedModelFieldName => $relatedModelField) {
						if ($relatedModelFieldName == Inflector::singularize($model->table) . '_count') {
							$_tmpCounterCache = true;
							continue;
						}
					}
				}
				$associations['belongsTo'][] = array(
					...
					'counterCache' => (!empty($_tmpCounterCache)) ? 'true' : 'false'
				);
 ...
 */
	public function bake($name, $data = array()) {
		...
		$defaults = array('associations' => array(), 'validate' => array(), 'primaryKey' => 'id',
			'useTable' => null, 'useDbConfig' => 'default', 'displayField' => null, 'isSlug' => null, 'isOrder' => null);
