 ...
 */
	protected function _interactive() {
		...
		$wannaDoAdmin = $this->in(__("Would you like to create the views for admin routing?"), array('y','n'), 'y');
...
// rajesh_04ag02 // 2008-06-30
		if (in_array($template, array('index'))) {
    		...
    		if ((strtolower($_isIndexList) == 'y' || strtolower($_isIndexList) == 'yes')) {
    			$template = 'index_list';
    		}
		}
		return $template;
...
// rajesh_04ag02 // 2008-07-07 // Was just array_keys()
				$associations[$type][$assocKey]['fields'] =  ($model->{$assocKey}->schema(true));
			}
		}
		return $associations;
	}
