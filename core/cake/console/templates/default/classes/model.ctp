...
foreach (array('hasOne', 'belongsTo') as $assocType):
	if (!empty($associations[$assocType])):
		...
		foreach ($associations[$assocType] as $i => $relation):
			...
			$out .= "\t\t\t'fields' => '',\n";
			$out .= "\t\t\t'order' => '',\n";
			$out .= "\t\t\t'counterCache' => {$relation['counterCache']}\n";
			$out .= "\t\t)";
			if ($i + 1 < $typeCount) {
				$out .= ",";
			}
...
if (!empty($validate)):
	...
	foreach ($validate as $field => $validations):
		...
		foreach ($validations as $key => $validator):
			echo "\t\t\t'$key' => array(\n";
			echo "\t\t\t\t'rule' => '" . $validator['rule'] . "',\n";
			if (!empty($validator['allowEmpty'])):
				echo "\t\t\t\t'allowEmpty' => " . $validator['allowEmpty'] . ",\n";
			endif;
			if (!empty($validator['message'])):
				echo "\t\t\t\t'message' => __l('" . $validator['message'] . "'),\n";
			endif;
			echo "\t\t\t),\n";
		endforeach;
		echo "\t\t),\n";
