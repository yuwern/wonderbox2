				...
				<?echo $this->Form->input('user_id', array('label' => __l('User'),'empty' => __l('Please Select')));
								...
								<? echo $this->Form->input('passwd', array('type' => 'password','label' => __l('New Password') , 'id' => 'new-password'));
							        <? echoecho $this->Form->input('confirm_password', array('type' => 'password', 'label' => __l('Confirm Password')));
								<? echoecho $this->Form->submit(__l('Update'),array('class'=>'btn1'));
								<? echoecho $this->Form->end();
							?>
                        </div>
                    </div>
				<?php if(empty($this->request->params['isAjax']) ): ?>	
